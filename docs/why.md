...
`go-ruby-activemodel/activemodel` reimplements Ruby's `activemodel` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-activemodel/activemodel` directly, with no Ruby runtime;
...
Because `activemodel` is pure Go with cgo disabled, it also compiles to **WebAssembly**
